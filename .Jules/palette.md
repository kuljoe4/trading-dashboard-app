...
## 2026-09-12 - DecisionLog Scroll-to-Top Button Keyboard Accessibility & Dynamic ARIA Standard
**Learning:** Floating action buttons and feed scroll triggers (such as the "New logs above ↑" scroll-to-top button in activity logs) frequently omit explicit button types (`type="button"`), high-contrast focus indicators, and dynamic screen reader descriptors. Supplying a dynamic `aria-label` detailing unread log counts (e.g., "Scroll to top (5 new logs above)") alongside high-contrast focus rings (`focus-visible:ring-2 focus-visible:ring-white`) ensures seamless keyboard and screen reader accessibility during real-time log feeds.
**Action:** Always enforce explicit `type="button"`, high-contrast focus-visible rings, and dynamic count-aware `aria-label` attributes on floating feed triggers.

## 2026-09-14 - Scanner Discovery Mode Filter Buttons ARIA-Pressed Accessibility Standard
**Learning:** Dense toolbar filter buttons in high-frequency monitoring overlays (such as the Discovery Mode buttons 'All Scanned', 'Top 24 Volume', 'Top 24h Change' in `ScannerOverlay.jsx`) often lack `aria-pressed` state attributes, explicit `aria-label` descriptions, and pointer cursors. Providing `aria-pressed={activeState}`, descriptive `aria-label` tags, explicit `type="button"`, and `cursor-pointer` styles ensures screen readers announce toggle filter states clearly and keyboard users experience standard WCAG 2.1 compliance.
**Action:** Always include `aria-pressed`, explicit `aria-label` strings, `type="button"`, and `cursor-pointer` on multi-state toolbar filter buttons across overlays and dashboards.

## 2026-09-15 - Predictive RR Calculator Header Button Semantics & Keyboard Accessibility
**Learning:** Expandable card headers (such as `RrWinRateCalculator` in `HistoryView.jsx`) that rely on plain `<div>` elements with `onClick` handlers and contain nested `<button>` controls violate semantic HTML standards and trap screen reader / keyboard navigators. Converting the outer header row into a semantic interactive button (`role="button"`, `tabIndex={0}`, `aria-expanded`, dynamic `aria-label`, `onKeyDown` for `Enter`/`Space`, `focus-visible:ring-2`) and replacing the nested `<button>` with a visual SVG wrapper establishes complete WCAG 2.1 keyboard and auditory accessibility parity.
**Action:** Always convert expandable card headers into focusable `role="button"` triggers with dynamic `aria-expanded` attributes, keyboard handlers, and focus-visible rings while avoiding nested interactive button elements.
