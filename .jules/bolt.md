## 2026-09-26 - [Learning] Cold-Path Micro-Optimizations Inside useMemo Hooks
**Learning:** Replacing functional JS methods (`Object.keys().forEach`) with direct `for...in` loops inside React `useMemo` hooks (such as `activeSignalKeys` in `TradeDetailContent.jsx`) provides minimal real-world performance gain (~1.08x) because `useMemo` already gates computation behind reference equality checks.
**Action:** Focus performance optimization effort on high-frequency unmemoized execution paths (such as price tick updates and store normalization loops) rather than cold memoized render paths.
