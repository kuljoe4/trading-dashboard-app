...
## 2026-06-27 - ConfigModal Mobile Layout Fix
**Learning:** Forcing manual scroll resets (`window.scrollTo(0,0)`) on mobile viewport resize conflicts with Drawer-based components and causes layout gaps upon keyboard dismissal.
**Action:** Removed aggressive scroll reset logic in `ConfigModal` and transitioned to using targeted `scrollIntoView` on input focus, allowing the Drawer component to manage viewport changes naturally.
