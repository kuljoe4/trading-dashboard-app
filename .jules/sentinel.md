## 2026-09-25 - Audit Log Retention Parameter Validation Standard
**Vulnerability:** In `AuditLogService.cleanup` (`backend/node/src/trading/audit-log.service.ts`), passing a negative, zero, or non-numeric `days` parameter caused the cutoff date `Date.now() - days * 86400000` to be evaluated in the future, executing `DELETE FROM audit_logs WHERE timestamp < cutoff` and wiping all current and historical audit logs from the database.
**Learning:** Functions accepting retention periods or time offsets for data deletion must strictly validate and clamp inputs to positive bounds to prevent negative offsets from shifting the cutoff into the future and purging valid current data.
**Prevention:** Always parse and clamp time-retention parameters to safe positive ranges (`1..3650`) with fallback defaults before building SQL deletion cutoffs.

## 2026-09-20 - SessionController Outer DTO Whitelist Validation Standard
**Vulnerability:** In `SessionController.updateTradeConfig` (`PATCH /session/trade/:id/config`) and `SessionController.updateSession` (`PATCH /session/:id`), incoming `@Body() body` payloads relied solely on NestJS route parameter binding without explicit outer `plainToInstance(UpdateTradeConfigDto, body || {})` or `plainToInstance(UpdateSessionDto, body || {})` transformation and `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` checks. This allowed non-whitelisted top-level request parameters to bypass defense-in-depth whitelist gating at the controller boundary.
**Learning:** In NestJS controllers handling partial updates or active session/trade modifications, top-level DTO payloads must be explicitly instantiated via `plainToInstance` and validated with `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` at the controller entry barrier to enforce defense-in-depth whitelist gating and prevent unauthorized parameter injection.
**Prevention:** Always perform explicit `plainToInstance(TargetDto, body || {})` and `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` on top-level request DTO payloads in controller endpoints before delegating to service execution methods.

## 2026-09-18 - AdoptPositionDto Whitelist Validation Standard
**Vulnerability:** In `SessionController.adoptPosition` (`POST /session/adopt-position`), incoming `@Body() body: AdoptPositionDto` was passed directly to `sessionService.adoptPositionManually` without explicit `plainToInstance(AdoptPositionDto, body || {})` transformation or `validate(adoptDto, { whitelist: true, forbidNonWhitelisted: true })` checks at the controller boundary. This allowed non-whitelisted properties or uninstantiated request parameters to bypass strict defense-in-depth whitelist gating before reaching manual position adoption logic.
**Learning:** In NestJS controllers handling manual state transitions or parameter adoption, incoming DTO payloads must be explicitly instantiated via `plainToInstance` and validated with `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` at the controller entry barrier to enforce defense-in-depth whitelist gating and prevent unauthorized property injection.
**Prevention:** Always perform explicit `plainToInstance(TargetDto, body || {})` and `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` on request DTO payloads in controller endpoints before delegating to service execution methods.
...
**Prevention:** Always invoke `validate(configInstance, { whitelist: true, forbidNonWhitelisted: true })` on instantiated configuration objects prior to passing them into backtesting or session execution pipelines.

## 2026-09-12 - SessionConfig Whitelist Validation on Update Trade Config Endpoint
**Vulnerability:** In `SessionController.updateTradeConfig` (`PATCH /session/trade/:id/config`), when `body.strategy_config` was passed to update trade strategy configuration overrides, it was passed to `sessionService.updateTradeConfig` without prior controller-level `plainToInstance(SessionConfig, body.strategy_config)` transformation or `validate(configInstance, { whitelist: true, forbidNonWhitelisted: true, skipMissingProperties: true })` checks. This allowed non-whitelisted or malformed strategy configuration parameters in HTTP PATCH requests to bypass strict whitelist gating before reaching trade service merge and database persistence logic.
**Learning:** In NestJS controller endpoints that accept partial strategy configuration overrides for active trades, incoming `strategy_config` payloads must be explicitly instantiated with `plainToInstance` and validated with `validate(configInstance, { whitelist: true, forbidNonWhitelisted: true, skipMissingProperties: true })` at the entry barrier before delegating to service methods.
**Prevention:** Always perform explicit `plainToInstance(SessionConfig, strategyConfig)` transformation and `validate(configInstance, { whitelist: true, forbidNonWhitelisted: true, skipMissingProperties: true })` on trade strategy override payloads in NestJS controller endpoints before executing service-layer trade updates.

## 2026-09-13 - PresetsController savePreset DTO Whitelist Validation
**Vulnerability:** In `PresetsController.savePreset` (`POST /presets`), incoming `@Body() body: CreateStrategyPresetDto` relied solely on NestJS `ValidationPipe` for outer DTO validation while manually validating `body.config`. This allowed non-whitelisted top-level properties or malformed preset request parameters to bypass strict schema whitelist checks before reaching database operations and audit logging.
**Learning:** In NestJS controllers handling strategy presets or compound payloads, outer DTO instances must be explicitly instantiated using `plainToInstance(CreateStrategyPresetDto, body || {})` and validated with `validate(presetDto, { whitelist: true, forbidNonWhitelisted: true })` alongside nested configuration validations to enforce strict defense-in-depth whitelist gating.
**Prevention:** Always perform explicit `plainToInstance(TargetDto, body || {})` and `validate(dtoInstance, { whitelist: true, forbidNonWhitelisted: true })` on outer DTO objects in controller handler methods before executing database queries or logging actions.

## 2026-09-16 - SettingsController DTO Whitelist Validation Standard
**Vulnerability:** In `SettingsController.updateKeys` (`POST /settings/keys`) and `SettingsController.validateKeys` (`POST /settings/keys/validate`), incoming request bodies relied solely on default NestJS parameter binding without explicit `plainToInstance` transformation or `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` checks at the controller handler barrier. This allowed non-whitelisted or malformed request parameters handling exchange API credentials to bypass explicit whitelist gating prior to encryption and database operations.
**Learning:** In NestJS controllers handling exchange credentials or sensitive system settings, incoming request payloads must be explicitly instantiated via `plainToInstance(TargetDto, body || {})` and validated with `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` at the controller entry barrier to enforce strict defense-in-depth whitelist gating and prevent unauthorized property injection.
**Prevention:** Always perform explicit `plainToInstance(TargetDto, body || {})` and `validate(dto, { whitelist: true, forbidNonWhitelisted: true })` on request DTO payloads in controller endpoints before executing sensitive operations or database persistence.

## 2026-09-23 - SessionController backfillKlines Audit Log Metadata Propagation Standard
**Vulnerability:** In `SessionController.backfillKlines` (`POST /session/backfill-klines`), incoming requests triggered external Binance Futures REST API queries without extracting client IP or User-Agent request metadata or logging an entry in `AuditLogService`. This created an audit gap where force-backfilling candles (an action that consumes exchange API weight and alters market feed state) lacked forensic accountability.
**Learning:** Controller endpoints triggering outbound exchange REST calls or state synchronization must capture client IP and User-Agent metadata and propagate them to service-level audit logging routines.
**Prevention:** Always extract `clientIp` and `userAgent` in controller route handlers for actions that consume external exchange weight or alter engine state, and record structured entries in `AuditLogService`.
## 2026-09-21 - PresetsController savePreset Nested Config Whitelist Validation Standard
**Vulnerability:** In `PresetsController.savePreset` (`POST /presets`), after validating `presetDto.config` using `validate(configInstance, { whitelist: true, forbidNonWhitelisted: true })`, the raw, untransformed `presetDto.config` object was saved to `preset.config` in `presetRepository`. This allowed un-whitelisted or raw non-decorated properties present on `presetDto.config` to bypass class-transformer instantiation and persist in database storage.
**Learning:** In NestJS controllers that validate nested configuration instances using `plainToInstance` and `validate`, the resulting instantiated and whitelisted `configInstance` must be directly assigned to entity properties when persisting to database storage rather than using the raw request body object.
**Prevention:** Always assign the transformed and validated DTO/class instance (e.g. `preset.config = configInstance`) to persistent entities before executing database save calls.
