...
describe('MomentumScannerService Environment Filtering', () => {
  ...
  it('rejects live orders for symbols without filters in OrderManagerService', async () => {
    ...
    const mockRest = {
      newOrder: jest.fn().mockResolvedValue({ data: { orderId: 'mock', avgPrice: '50000', executedQty: '1' }, headers: {} }),
      newAlgoOrder: jest.fn().mockResolvedValue({ data: { algoId: 'mock-sl', algoStatus: 'NEW' }, headers: {} }),
      cancelAllOpenOrders: jest.fn().mockResolvedValue({ data: { code: 0 }, headers: {} }),
    ...
    mockRest.newOrder.mockResolvedValue({ data: { orderId: 'mock', avgPrice: '1', executedQty: '100' }, headers: {} });
    mockRest.newAlgoOrder.mockResolvedValue({ data: { algoId: 'mock-sl', algoStatus: 'NEW' }, headers: {} });
