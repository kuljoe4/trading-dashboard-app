      ...
      for (const trade of tradesToAudit) {
        try {
          ...
          if (!pos || Math.abs(parseFloat(pos.positionAmt)) === 0) {
              ...
              this.eventEmitter.emit('trade.exchange_close', { symbol: trade.symbol, exitPrice: 0, reason: ENGINE_CONSTANTS.EXIT_REASONS.EXCHANGE_SYNC });
              continue;
          }
          ...
          if (!matchingOrder) {
            ...
            if (secondsSinceUpdate > 120) {
              ...
              this.eventEmitter.emit('trade.exchange_close', { symbol: trade.symbol, exitPrice: 0, reason: ENGINE_CONSTANTS.EXIT_REASONS.WATCHDOG_NUCLEAR_CLOSE });
              continue;
            }
