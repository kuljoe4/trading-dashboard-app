...
export const Sidebar = ({ selected }) => {
  const { wsStatus, sidebarCollapsed: collapsed, toggleSidebar, monitoring, rateLimit, gateState } = useTradingStore()
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <SystemMetrics monitoring={monitoring} rateLimit={rateLimit} wsStatus={wsStatus} gateState={gateState} compact={!isExpanded} />
      </div>
    </div>
  )
}
...
export const BottomNav = ({ selected }) => {
  const { wsStatus, monitoring, rateLimit, gateState } = useTradingStore()
  const isActive = (path) => {
    if (path === '/') return !selected && window.location.hash === '#/'
    return window.location.hash.startsWith(`#${path}`)
  }
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-surface/90 backdrop-blur-md border-t border-border z-40">
      ...
      <div className="px-4 pb-2 border-t border-border/40 flex justify-center pt-2">
        <SystemMetrics monitoring={monitoring} rateLimit={rateLimit} wsStatus={wsStatus} gateState={gateState} compact={true} />
      </div>
    </div>
  )
}
