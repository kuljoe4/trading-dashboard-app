    ...
    <div className="flex flex-col gap-4 md:gap-6">
      ...
      <div className="flex flex-col gap-1.5 md:gap-2">
        ...
        <div className="relative pt-3 pb-0.5 min-h-[26px]">
          ...
          {/* Active Trailing Stop Loss Flag Marker on Runway Track */}
          {(trade.strategy_config?.trailing_stop_enabled || trade.strategy_config?.sl_type === 'trailing') && sl > 0 && (() => {
            ...
            if (tp > 0) {
              const totalRange = isLong ? (tp - sl) : (sl - tp);
              const distFromSl = isLong ? 0 : totalRange;
              trailingSlPos = Math.max(0, Math.min(100, (distFromSl / totalRange) * 100));
            }
            ...
            return (
              <Tooltip content={
                <div className="flex flex-col gap-1 text-[11px] p-1 font-sans text-left">
                  ...
                  <div className="text-dim">Trailing SL Price: <span className="text-purple-300 font-mono font-semibold">{fmtUSD(sl)}</span></div>
                ...
                <div
                  className="absolute top-0 bottom-0 z-25 cursor-help transition-all duration-300 flex flex-col items-center -ml-[1px]"
                  style={{ left: `${trailingSlPos}%` }}
                >
                  <div className="px-1 py-0.2 text-[6.5px] font-black uppercase rounded tracking-tighter shadow-[0_0_8px_rgba(168,85,247,0.8)] mb-0.5 leading-none transition-all duration-300 flex items-center gap-0.5 bg-purple text-white animate-pulse">
                    🛡️ TRAIL
                  </div>
