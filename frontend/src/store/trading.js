...
export const useTradingStore = createWithEqualityFn((set, get) => ({
  ...
  entryCount: 0, hitCount: 0, peakRr: 0,
  alerts: [],
  ...
  connectWS: () => {
    ...
    ws.onmessage = (e) => {
      ...
      if (d.type === 'status' || d.type === 'session') {
        ...
        set(st => ({ activeTrades: d.event === 'closed' ? st.activeTrades.filter(x => x.symbol !== d.symbol) : (t ? [...st.activeTrades, t] : st.activeTrades), tradeHistory: d.event === 'closed' && t ? [t, ...st.tradeHistory].slice(0, 50) : st.tradeHistory, entryCount: d.stats?.entryCount ?? st.entryCount, hitCount: d.stats?.hitCount ?? st.hitCount, peakRr: d.stats?.peakRr ?? st.peakRr }));
