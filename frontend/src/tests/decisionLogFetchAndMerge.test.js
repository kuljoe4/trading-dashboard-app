...
test('DecisionLog Fetch, Limit Selectors & Non-Destructive Log Merge Standard', () => {
  const componentPath = path.join(process.cwd(), 'frontend/src/components/DecisionLog.jsx')
  ...
  const clientPath = path.join(process.cwd(), 'frontend/src/api/client.js')
  ...
  const storePath = path.join(process.cwd(), 'frontend/src/store/trading.js')
